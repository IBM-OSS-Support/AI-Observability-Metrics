...
def run_chat_model(user_id, question):
    try:
        ...
        with graphsignal.start_trace('predict', options=graphsignal.TraceOptions(record_samples= True, record_metrics=True, enable_profiling=True)):
            print(runnable.stream({"question": question}))
            output = ""
...
if __name__ == "__main__":
    os.environ['OPENAI_API_KEY'] = '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'
    inject_roja_instrumentation(APPLICATION_NAME, USER)
    #run_chat_model("tahsin",PROMPT)
    log_prompt_info(PROMPT)