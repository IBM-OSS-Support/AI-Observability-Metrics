  ...
  meter: {
    proportional: {
      ...
      breakdownFormatter: (e) =>
        `The rating of the application is ${e.datasetsTotal.toFixed(
          2
        )} out of 5`,
    },
    height: "60%",
...
const UserSatisfaction = forwardRef(
  ({ selectedItem, selectedUser, startDate, endDate }, ref) => {
    ...
    const sendMessageToServerUser = async (
      selectedItem,
      selectedUser,
      startDate,
      endDate
      ...
      try {
        ...
        setMessageFromServerUser(responseData);
      } catch (error) {
    ...
    useEffect(() => {
      if (messageFromServerUser && state.status === "success") {
        ...
        const newAvgValue =
          userRatings.reduce((s, g) => s + +g, 0) / userRatings.length;
        ...
        {loading ? (
          ...
          <Tile className="infrastructure-components accuracy p-0">
            ...
            <div className="cpu-usage-data">
              {avg > 0 ? (
                <>
                  <div className="label">Average rating of application</div>
